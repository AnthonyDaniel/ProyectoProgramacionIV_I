...
class AdminController extends Controller
{
    ...
    public function addParking(RParkingLot $request)
    {
        try {
            ...
            for ($i = 0; $i < $request->cantidad; $i++) {
                DB::insert('INSERT INTO espacio(idEspacio,parqueo,zona) VALUES(?,?,?)', [
                    ($i + 1) . ":" . $request->zona,
                    $request->idParqueo,
                    $request->zona
                ]);
    ...
    {
        try {
            $resut = DB::select("SELECT * FROM espacio WHERE zona=?", [$request->zona]);
    ...
    public function deleteSpace(Request $request)
    {
        try {
           // DB::insert('DELETE FROM espacio where idEspacio= ?', [$request->idEspacio]);
            $e = DB::select('SELECT count(idEspacio) as t FROM espacio Where zona=?', [$request->zona]);
